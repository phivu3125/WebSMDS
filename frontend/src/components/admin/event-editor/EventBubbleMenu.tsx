...
import { EditorButton, EditorButtonGroup } from '../editor/EditorButton'
import {
  Bold,
  Italic,
  Underline,
...
interface EventBubbleMenuProps {
  editor: Editor | null
}

export function EventBubbleMenu({ editor }: EventBubbleMenuProps) {
  ...
  const updateBubbleMenuPosition = (pos: number) => {
    ...
    // Calculate position as percentage relative to editor element
    const relativeX = ((coords.left - editorRect.left) / editorRect.width) * 100
    const relativeY = coords.top - editorRect.top - 10 // Position above selection relative to editor

    setBubbleMenuPosition({
      x: relativeX, // Position as percentage
      y: relativeY // Position in pixels for vertical
    })
  }
